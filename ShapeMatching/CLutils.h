...
#define _USE_GL_INTEROP
#ifdef _USE_GL_INTEROP
...
namespace cl {
	...
	//================
	template< class T >
	vec3<T> Clamp(vec3<T> v, vec3<T> min, vec3<T> max) {
		vec3<T> res = v;
		if(v.x > max.x)  res.x = max.x;
		if(v.y > max.y)  res.y = max.y;
		if(v.z > max.z)  res.z = max.z;

		if(v.x < min.x)  res.x = min.x;
		if(v.y < min.y)  res.y = min.y;
		if(v.z < min.z)  res.z = min.z;
		return res;
	}

	vec3<int> Convert_FloatToInt(vec3<float> v) {
		return vec3<int>(static_cast<int>(v.x), static_cast<int>(v.y), static_cast<int>(v.z));
	}
	//================
	// Error
	//================
	...
	public:
		...
		void GetWorkGroupInfo(Program &program, size_t &size){
			Error(clGetKernelWorkGroupInfo(_kernel, program._device_id, CL_KERNEL_WORK_GROUP_SIZE, sizeof(size_t), &size, NULL), "clGetKernelWorkGroupInfo");
		}
	...
	public:
		...
		GLMem(Context &context, cl_mem_flags flags, cl_GLuint &bufobj) : _mem(0) { // FIXME : _mem( 0 )
			CreateBuffer(context, flags, bufobj);
		}
	...
	public:
		...
		void CreateBuffer(Context &context, cl_mem_flags flags, cl_GLuint &bufobj) {
			Release();
			cl_int err;
			_mem = clCreateFromGLBuffer(context._context, flags, bufobj, &err); Error(err, "clCreateGLBuffer");
		}
	...
	public:
		...
		GLMem(const GLMem &mem) {
			*this = mem;
		}

		GLMem &operator=(const GLMem &mem) {
			if (_mem == mem._mem) {
				return *this;
			}
